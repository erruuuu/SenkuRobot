...
async def download_video(v_url):
    ...
    type = "audio"
    await rkp.edit("`Persiapan untuk mendownload...`")
    if type == "audio":
        opts = {
            'format':
            'bestaudio',
        ...
        song = True    
    try:
        await rkp.edit("`Mengambil data, harap tunggu..`")
    ...
    c_time = time.time()
    if song:
        await rkp.edit(f"`Bersiap mengunggah lagu:`\
        \n**{rip_data['title']}**\
...
__help__ = """
 ➩ /song2 <songname artist(optional)>: uploads the song in it's best quality available

 ➩ /video2 <songname artist(optional)>: uploads the video song in it's best quality available
"""
